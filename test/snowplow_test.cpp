...
TEST_CASE("snowplow") {
  ...
  SECTION("setting default tracker") {
    ...
    auto emitter3 = make_shared<Emitter>(NetworkConfiguration("https://collector.com"), emitter_config3);
  ...
  SECTION("registers a custom tracker") {
    ...
    auto emitter = make_shared<Emitter>(NetworkConfiguration("https://collector.com"), emitter_config);
  ...
  SECTION("handles multiple trackers") {
    ...
    EmitterConfiguration emitter_config1(storage1);
    emitter_config1.set_http_client(move(unique_ptr<TestHttpClient>(http_client1)));
    auto tracker1 = Snowplow::create_tracker(
      TrackerConfiguration("ns1"),
      NetworkConfiguration("http://com.acme.collector", POST),
      emitter_config1
    );

    auto http_client2 = new TestHttpClient();
    EmitterConfiguration emitter_config2(storage2);
    emitter_config2.set_http_client(move(unique_ptr<TestHttpClient>(http_client2)));
  ...
  SECTION("creates a common session and event store when given the same database path") {
    ...
    auto tracker = Snowplow::create_tracker(
        TrackerConfiguration("ns"), NetworkConfiguration("http://com.acme.collector"), emitter_config, session_config);
