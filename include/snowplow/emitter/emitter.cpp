...
void Emitter::stop() {
  ...
  if (this->m_running == true) {
    ...
    // Set m_stop_requested under m_db_select to eliminate the lost-wakeup race:
    // run() checks m_stop_requested while holding m_db_select before entering idle
    // wait_for, so setting it here under the same lock guarantees the check sees it.
    {
      lock_guard<mutex> db_lock(m_db_select);
...
void Emitter::run() {
  do {
    ...
    if (event_rows.size() > 0) {
      ...
      // Check m_stop_requested while holding m_db_select to avoid lost-wakeup: if
      // stop() sets m_stop_requested under this same lock before we reach wait_for,
      // we see it here and skip the sleep.  Otherwise stop() will notify us out of
      // the non-predicated wait_for (any notify wakes it, including from add()).
