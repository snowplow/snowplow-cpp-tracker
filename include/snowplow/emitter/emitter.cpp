...
void Emitter::stop() {
  ...
  if (this->m_running == true) {
    ...
    // Set m_stop_requested under m_db_select to close the idle-sleep lost-wakeup race:
    // run() holds m_db_select while checking m_stop_requested and entering wait_for, so
    // we must hold m_db_select while setting it to guarantee run() sees the signal.
    {
...
void Emitter::run() {
  do {
    ...
    if (event_rows.size() > 0) {
      ...
      // sleep for the retry delay if there is one; use predicated wait so stop() interrupts it
      ...
      // idle sleep - check stop signal first (under lock) to avoid lost-wakeup with stop()
