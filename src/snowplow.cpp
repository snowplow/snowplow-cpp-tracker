...
shared_ptr<Tracker> Snowplow::create_tracker(const string &name_space, const string &collector_url, Method method, shared_ptr<EventStore> event_store, shared_ptr<Subject> subject) {
  ...
  return move(create_tracker(
      ...
      NetworkConfiguration(collector_url, method),
      emitter_config,
      move(subject)));
}
...
shared_ptr<Tracker> Snowplow::create_tracker(const TrackerConfiguration &tracker_config, const NetworkConfiguration &network_config, EmitterConfiguration &emitter_config, shared_ptr<Subject> subject) {
...
shared_ptr<Tracker> Snowplow::create_tracker(const TrackerConfiguration &tracker_config, const NetworkConfiguration &network_config, EmitterConfiguration &emitter_config, SessionConfiguration &session_config, shared_ptr<Subject> subject) {
